...
- The initial Repair Cycle limit is three repairs per failed Candidate Program, configurable and available for 0/1/3 ablation. Repeated AST, repeated failure without improvement, and budget exhaustion stop the cycle early.
...
- V2 initially uses Structured Retrieval without embeddings. Compatibility filtering precedes Failure Type/Reason buckets, deterministic state and AST feature ranking, and explicit tie-breaking.
...
- Exactly one final Candidate Program is selected before held-out evaluation using Attempt Outcome, search-seed success proportion, mean and worst Normalized Progress, lower episode cost, and stable identity.
...
- Replicates use paired Task seeds, search RNG seeds, model settings, and budgets. Replicate counts and confidence interval methods are fixed from pilot variance before formal evaluation.
